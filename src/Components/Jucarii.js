...
const Jucarii = () => {
  ...
  useEffect(() => {
    const getAllData = async () => {
      try {
        //de schimbat
        const response = await fetch("/api/sell/jucarii");
  ...
  function fly() {
    document.querySelector(".all-parent-div").classList.add("all-fly");
  }
  return (
    <div>
      {alldata.map((item) => {
          ...
          <div key={item._id} className="all-parent-div">
            ...
            <div className="all-main">
              ...
              <div className="all-footer">
                ...
                {user.username === item.username && (
                  <Icon icon="fluent:delete-16-filled" className="all-delete" onClick={fly}/>
                )}
